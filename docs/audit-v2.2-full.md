...
- **Observation:** The CI security workflow uses `curl -sk` when uploading Semgrep, Trivy, and Gitleaks scan results to `https://defectdojo.internal.homelab.equipment`. The `-k` flag disables TLS certificate verification.
